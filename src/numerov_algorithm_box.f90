...
CONTAINS
  ...
  FUNCTION wf_numerov_lr(energy)
    ...
    REAL(KIND = DP)  :: yp_left, yp_right, gn_min, gn_max
    ...
    gn_max = gn(npoints)
    !
    ! wf at rigid box is zero
    y_left(1) = 0.0_DP
    yp_left = 1.0_DP
    !
    y_left(2) = x_step*yp_left
    !
    ...
    !
    !
    y_right(npoints) = 0.0_DP 
    !
    IF (MOD(nodes_num,2) == 0) THEN
       yp_right = 1.0_DP ! even case
    ELSE
       yp_right = -1.0_DP ! odd case
    ENDIF
    !
  ...
  SUBROUTINE wf_numerov(energy, wf_vector)
    ...
    REAL(KIND = DP)  :: yp_left, yp_right, gn_min, gn_max, norm_con
